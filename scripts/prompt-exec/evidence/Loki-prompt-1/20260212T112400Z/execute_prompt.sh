...
PROMPT_PATH='/home/luce/apps/loki-logging/_build/Sprint-1/Prompts/Loki-prompt-1.md'
RUN_DIR='/home/luce/apps/loki-logging/scripts/prompt-exec/evidence/Loki-prompt-1/20260212T112400Z'
...
for f in "$RUN_DIR/blocks"/block*.sh; do
  ...
  # Carry forward Phase 0 evidence root for prompts that expect EVID in later blocks.
...
if [ "$MOVE_TO_COMPLETED" != "0" ]; then
  COMPLETED_DIR="$PROMPT_DIR/completed"
  mkdir -p "$COMPLETED_DIR"

  DEST="$COMPLETED_DIR/$PROMPT_BASE"
  if [ -e "$DEST" ]; then
    DEST="$COMPLETED_DIR/$PROMPT_STEM.$RUN_UTC.md"
  fi
