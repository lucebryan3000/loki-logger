| 16 | **Quality & Test Coverage** | ADR-022, ADR-023, ADR-024, ADR-031, ADR-044 | 5 |
| 17 | **Accuracy Corrections (Pass 14)** | CORRECTION-001 – CORRECTION-013 | 13 |
...
### CORRECTION-009: Recording Rules `loki_ingestion_errors` — Fix NOT Applied
...
- **Status**: ADR-094 Codex action (add `or vector(0)`) and removal of `LokiIngestionErrors` are **STILL PENDING**. No fix has been applied to either file.
...
### CORRECTION-011: `docs/reference.md` — Resource Limits Table Stale
...
### ADR-134: Loki — No `query_range` Block (No Query Splitting)

**Domain**: Loki  
**Severity**: Medium  
**Status**: Open

**Evidence** — grep for query_range/split_queries in `infra/logging/loki-config.yml`:

```
(no output)
```

**Best Practice**: Without a `query_range` block, Grafana dashboard queries spanning multiple days execute as single unsharded queries against Loki. On a single-node deployment with 30-day retention, a 7-day query scans all 7 days in one pass and can spike memory. Best practice:
...
`cache_results: false` is acceptable for single-node filesystem storage where cache backend setup is not warranted.

**Deviation**: No `query_range` block. All queries execute unsplit.

**Recommended Remediation**: Add the `query_range` block to `loki-config.yml`.

---

### ADR-135: Loki — `common.path_prefix` Used Without Explicit Component Paths

**Domain**: Loki  
**Severity**: Low  
**Status**: Open

**Evidence** — `infra/logging/loki-config.yml` lines 6-7:
...
Storage chunks path is explicitly set (`/loki/chunks`). WAL path is not configured at all (see ADR-129). Rules path is implicit (would derive to `/loki/rules`).

**Best Practice**: Use `common.path_prefix` as a base but declare every component path explicitly so the config is self-documenting and immune to Loki version changes in path derivation logic.

**Deviation**: Partial — chunks explicit, WAL path not configured, rules path implicit.

**Recommended Remediation**: When adding `ingester.wal.dir` (ADR-129), use `/loki/wal` explicitly rather than relying on prefix derivation.
...
- record: sprint3:loki_ingestion_errors:increase10m
  expr: sum(increase(loki_write_dropped_entries_total[10m])) + sum(increase(loki_write_failures_discarded_total[10m]))
```
...
**Deviation**: Not fixed. Both recording rules retain `sum(A) + sum(B)` pattern. CORRECTION-009 confirmed this remains unfixed.
...
Apply same fix to `increase10m` variant.
...
### ADR-152: Grafana — Dashboard Provisioning Permits Runtime Edits and Deletions

**Domain**: Grafana  
**Severity**: Low  
**Status**: Open

**Evidence** — `infra/logging/grafana/provisioning/dashboards/dashboards.yml`:
...
**Best Practice**: `disableDeletion: false` allows provisioned dashboards to be deleted from the Grafana UI. `editable: true` allows provisioned dashboards to be edited via the UI. Both settings can cause runtime state to diverge from the provisioned state on disk. In a GitOps model, provisioned dashboards should be immutable.

**Deviation**: Both settings permissive. Dashboards can be deleted or edited outside of source control.

**Recommended Remediation**: For production governance:
...
For active development, current settings are acceptable but should be tightened before production handover.
...
|-----|--------|---------|----------|--------|
| ADR-129 | Loki | No ingester block — no WAL config, no chunk tuning | Medium | Open |
| ADR-130 | Loki | Chunk encoding not declared (relies on default snappy) | Low | Open |
| ADR-131 | Loki | No query-side limits (max_entries_limit, query_timeout) | Medium | Open |
...
| ADR-134 | Loki | No `query_range` block — no query splitting for 30d window | Medium | Open |
| ADR-135 | Loki | `common.path_prefix` without explicit per-component paths | Low | Open |
| ADR-136 | Alloy | `loki.write "default"` — no backoff or retry config | High | Open |
| ADR-137 | Alloy | `loki.write "default"` — no batch_size or batch_wait | Low | Open |
| ADR-138 | Alloy | `loki.write "default"` — no external_labels | Low | Open |
| ADR-139 | Alloy | `discovery.docker "all"` — no refresh_interval (defaults 5s) | Low | Open |
| ADR-140 | Alloy | No stage.drop / stage.limit for noisy sources | Medium | Open |
| ADR-141 | Alloy | No stage.metrics for pipeline observability | Low | Open |
| ADR-142 | Alloy | No stage.multiline for stack trace handling | Medium | Open |
| ADR-143 | Alloy | No stage.decolorize for ANSI code stripping | Low | Open |
| ADR-144 | Prometheus | No alertmanager_configs — alerts fire but never route | High | Open |
| ADR-145 | Prometheus | scrape_timeout not declared (implicit 10s) | Low | Open |
| ADR-146 | Prometheus | Recording rules sum(A)+sum(B) no-data risk (unfixed) | Medium | Open |
| ADR-147 | Prometheus | Alert rules have no runbook_url annotations | Low | Open |
| ADR-148 | Prometheus | No metric_relabel_configs to reduce cAdvisor cardinality | Low | Open |
| ADR-149 | Grafana | No contact point or notification policy provisioned | High | Open |
| ADR-150 | Grafana | noDataState: OK on e2e-marker rule — blind spot when Loki down | High | Open |
| ADR-151 | Grafana | Alert rule A→C missing reduce step B (unfixed) | Medium | Open |
| ADR-152 | Grafana | Dashboard provisioning allows runtime edits + deletions | Low | Open |
| ADR-153 | Grafana | updateIntervalSeconds not set (defaults to 10s) | Low | Open |
| ADR-154 | Grafana | Cookie security not explicitly configured | Medium | Open |
| ADR-155 | Grafana | Anonymous access / user signup not explicitly locked | Low | Open |
| ADR-156 | Compose | No container logging driver limits — uncapped json-file logs | Medium | Open |
| ADR-157 | Compose | No security_opt: no-new-privileges on any container | Medium | Open |
| ADR-158 | Compose | No ulimits.nofile on Loki service | Medium | Open |
| ADR-159 | Compose | No read_only root filesystem on any container | Low | Open |

**High severity** (4): ADR-132, ADR-136, ADR-144, ADR-149, ADR-150  
**Medium severity** (11): ADR-129, ADR-131, ADR-133, ADR-134, ADR-140, ADR-142, ADR-146, ADR-151, ADR-154, ADR-156, ADR-157, ADR-158  
**Low severity** (16): ADR-130, ADR-135, ADR-137, ADR-138, ADR-139, ADR-141, ADR-143, ADR-145, ADR-147, ADR-148, ADR-152, ADR-153, ADR-155, ADR-159
